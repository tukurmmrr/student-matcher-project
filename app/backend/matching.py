...
"""
 File for Matching algorithms for students based on interests and courses
"""

import models

def create_admin_profile_dict(student: models.Student):
    # helper to create dict for admin view
    return {"name": student.name, "course": student.course.name if student.course else "N/A"}

def calculate_jaccard_for_admin(students):
    """
    Calculate Jaccard similarity for all student pairs
    Jaccard = intersection / union
    """
    # create sets of interests for faster lookup
...
def calculate_dice_for_admin(students):
    """
    Dice coefficient similarity - different from jaccard
...
def calculate_matches_for_user(students, current_user_id):
    """
    Get matches for a specific user using jaccard similarity
    """
    matches = []
